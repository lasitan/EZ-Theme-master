  ...
  setup() {
    ...
    const preloadVueComponents = () => {
      ...
      setTimeout(() => {
        ...
        const pathKey = route.path && route.path.startsWith('/login') ? '/login' : route.path;
        if (pathKey in componentsConfig.route) {

          if (isCustomerServiceEnabled && pathKey !== '/customer-service') {

            componentsConfig.route[pathKey].unshift(customerServiceComponent);

          }

          enqueueComponents(componentsConfig.route[pathKey]);
    ...
    watch(() => route.path, (newPath, oldPath) => {
      ...
      const newKey = newPath && newPath.startsWith('/login') ? '/login' : newPath;
      const oldKey = oldPath && oldPath.startsWith('/login') ? '/login' : oldPath;
      if (newKey !== oldKey && newKey in componentsConfig.route) {
        

        if (isCustomerServiceEnabled && newKey !== '/customer-service' && !componentsConfig.route[newKey].some(comp => comp.name === 'CustomerService')) {

          componentsConfig.route[newKey].unshift(customerServiceComponent);

        }

        enqueueComponents(componentsConfig.route[newKey]);
